  ...
  server.registerTool(
    ...
    async ({ owner, projectNumber }) => {
      try {
        ...
        return { content: [{ type: "text" as const, text: formatGitHubError(error) }], isError: true };
      }
    },
  );
  ...
  server.registerTool(
    ...
    async ({ repo, title, body, labels }) => {
      try {
        ...
        return { content: [{ type: "text" as const, text: jsonText(issue) }] };
      } catch (error) {
        return { content: [{ type: "text" as const, text: formatGitHubError(error) }], isError: true };
      }
    },
