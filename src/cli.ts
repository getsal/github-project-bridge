...
function buildImportMarkdown(report: Awaited<ReturnType<GitHubProjectsBridge["importCsv"]>>): string {
  const lines = [
    "# Import Report",
  ...
  for (const row of report.rows) {
    ...
    if (row.issue) lines.push(`  - issue: #${row.issue.number} ${row.issue.url}`);
    if (row.projectItemId) lines.push(`  - projectItemId: ${row.projectItemId}`);
